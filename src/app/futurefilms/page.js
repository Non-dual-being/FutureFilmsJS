...
export default function FutureFilmsPage() {
  const THEMES = ['Mobiliteit', 'Energietransitie', 'Klimaatverandering', 'Watermanagement', 'Grondstoffen', 'Voedselinnovatie', 'Biodiversiteit', 'InternetofThings'];
  const LEVELS = ['1', '2', '3'];
  ...
  const isLevelAvailable = (level) => THEMES.every(theme => beschikbareVideos[theme]?.[level]?.length > 0);
  ...
  const handleCheckboxChange = (e) => {
    ...
    if (updating) return; // Prevent re-entry if already updating

    setSelectedCheckboxes(prev => {
      let newSelected = checked ? [...prev, id] : prev.filter(selectedId => selectedId !== id);
